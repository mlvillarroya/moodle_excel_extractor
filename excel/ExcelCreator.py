...
class ExcelCreator:
    ...
    def __init__(self,createMultipleChoice = False, createTrueFalse = False, createOneAnswer = False, createNumeric = False):
        ...
        if createMultipleChoice: 
            self.createMultipleChoiceSheet()
        if createOneAnswer: self.wb.create_sheet("One answer")
        if createTrueFalse: self.wb.create_sheet("True false")
        if createNumeric: self.wb.create_sheet("Numeric")
        self.wb.save('MoodleExcel.xlsx')

    def createSettingsSheet(self,ws):
        ws['A1'] = "Subject name"

    def createMultipleChoiceSheet(self):
        ws = self.wb.create_sheet("Multiple choice")
        ws['A1'] = "Question *"
